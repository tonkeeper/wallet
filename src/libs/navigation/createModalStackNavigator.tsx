...
import { DefaultNavigatorOptions, EventMapBase, NavigationState, ParamListBase, RouteConfig, RouteGroupConfig, RouteProp } from "@react-navigation/native";
import { createNativeStackNavigator, NativeStackNavigationOptions } from "@react-navigation/native-stack";
import { SheetActions, SheetsProvider, SheetsProviderRef } from './components/Modal/Sheet/SheetsProvider'
...
import { AdditionalRouteConfig, ModalBehavior, ModalRouteConfigByType, WithPath } from "./types";
...
const getRouteConfigsFromChildren = (children: React.ReactNode, props?: any) => {  
  const configs = React.Children.toArray(children).reduce<ModalRouteConfigByType>((acc, child) => {
    if (React.isValidElement(child)) {
      ...
      if (child.type === React.Fragment || child.type === ModalGroup) {
        ...
        const groupConfig = getRouteConfigsFromChildren(
          children,
          restProps
        );
...
export function createModalStackNavigator() {
  ...
  return {
    Navigator: createModalNavigator(Stack),
    Group: ModalGroup,
    Modal: ModalScreen,
  };
}
...
type ModalNavigatorProps = Omit<DefaultNavigatorOptions<{}, any, {}, {}>, 'screenOptions'> & {
  screenOptions?: NativeStackNavigationOptions
};

function createModalNavigator(Stack: any) {
...
>(_: WithPath<RouteConfig<ParamList, RouteName, State, ScreenOptions, EventMap>> & AdditionalRouteConfig) {
  return null;
}
